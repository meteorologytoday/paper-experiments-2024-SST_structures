...
ref_WRF_root = "/home/t2hsu/models/WRF_gcc/SST_structure_paper/WRFV4.6.0_FULL"
            ...
            for m, _bl_scheme in enumerate(bl_scheme):
                for n, _mp_heating in enumerate(mp_heating):
                    ...
                    sim_cases.append({
                        ...
                        'casename_short' : "m%sw%03dU%02ddT%03d%s" % (_mp_heating, wnm, U, dT*1e2, _bl_scheme),
                        'T0'         : T0,
                        'dT'         : dT,
                        'wnm'        : wnm,
                        'dx'         : dx,
...
for i, sim_case in enumerate(sim_cases):
    ...
    if os.path.isdir(case_folder):
        ...
        pleaseRun("f90nml -p -g physics -v bl_pbl_physics=%d -v sf_sfclay_physics=%d  -v no_mp_heating=%d -v ra_lw_physics=%d -v ra_sw_physics=%d -v mp_physics=1 %s/namelist_unmodified.input > %s/namelist.tmp" % (
